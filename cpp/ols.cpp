	...
	for(nstring arg: ag.get_arguments())
	{
		...
		try {
			...
			if (pt == "File") {
				...
				{
					if (ls.long_format) {
						std::time_t mtime_sec = (entry.wtime / 10000000) - 11644473600;
						std::tm* mtime = std::localtime(&mtime_sec);

						if (curr_time - mtime_sec < 15724800)
							cout << attribute_str(entry.attrib) << " " << setfill(' ') << setw(10) << entry.size << " " << setw(2) << mtime->tm_mday << " " <<  months[mtime->tm_mon] << " " << setfill('0') << setw(2) << mtime->tm_hour << ":"  << setw(2) << mtime->tm_min << " " << entry.name << endl;
						else
							cout << attribute_str(entry.attrib) << " " << setfill(' ') << setw(10) << entry.size << " " << setw(2) << mtime->tm_mday << " " <<  months[mtime->tm_mon] << " " << setw(5) << mtime->tm_year + 1900 << " " << entry.name << endl;
					} else {
